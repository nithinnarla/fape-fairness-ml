...
Scripts and notebooks named `stage2_*` hold Stage 3, the fairness intervention. The prefix is left over from the numbering used while the project was being built, and the dated decision records in `docs/` keep it.
...
**Verified: 4,961,848 distinct records.** Law School Admissions (law_school_lequy) and MEPS Panel 19 are both sub-datasets of the FairGround corpus, so their 18,692 and 15,830 records are already inside FairGround's 1,964,010 and are counted once. The corpus loads 37 of the 38 datasets fairml-datasets lists without the large-dataset option; law_school_tensorflow has returned HTTP 403 throughout. Two of them, law_school_tensorflow and stop_question_and_frisk_data, cannot be prepared from scratch in the pinned environment because fairml-datasets 0.2.5 calls np.bool, which numpy 1.26 removed, so a clone without this repository's cache/ loads 36 datasets and 1,955,063 records. MIMIC-III was never obtained and is not part of the study. USDA NASS is used descriptively in EDA only and LSMS-ISA Nigeria was excluded from modeling (Decision 11); both are counted here as collected.
...
- FairGround Corpus: 1,964,010 records across the 37 datasets that load, out of the 38 fairml-datasets 0.2.5 lists without the large-dataset option (Simson et al. 2025)
...
- June 2026: EDA and baseline modeling complete across all 7 domains (COMPAS, Folktables, Law School, Lending Club, Agricultural, FairGround, Student); logistic regression, random forest and gradient boosting trained and evaluated; 83 EDA figures committed
- July 2026: ThresholdOptimizer DP/EO constraints complete across all 7 domains; cross-domain comparison complete; Law School strongest improvement (EO +98.7%); Agricultural counterproductive when near-fair (DPD -244.4%); core finding: effectiveness tracks baseline DPD, with documented exceptions; 68 figures committed (56 domain + 6 aggregation + 6 cross-domain)
- July 2026: DIR metric added where each domain's pipeline supports it (before-and-after for Law School, Lending Club, Agricultural; baseline only for Folktables); paper outline committed
...
**Stages 1 and 2 (complete):** preprocessing, EDA and baseline models across the original seven domains, with 83 EDA figures and 68 baseline figures. MEPS was added later and evaluated through the FairGround pipeline, so it has no separate EDA set.
**Stage 3 (complete):** ThresholdOptimizer fairness interventions across all eight evaluations and a cross-domain comparison, with 70 figures: 56 from the seven domain scripts, 8 aggregation and 6 cross-domain.
The disparate impact ratio is computed where the domain pipeline supports it: a fixed-pair ratio before and after for Law School, Lending Club and Agricultural, and per-race ratios under the equalized odds constraint for Folktables.
